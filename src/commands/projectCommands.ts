...
export function registerCreateProjectCommand(): vscode.Disposable {
    return vscode.commands.registerCommand(EXTENSION_CONSTANTS.COMMANDS.CREATE_PROJECT, async () => {
        ...
        try {
            for (const folder of EXTENSION_CONSTANTS.FOLDERS.LIST) {
            ...
            const workfieldsPath = path.join(
                projectPath, 
                EXTENSION_CONSTANTS.FOLDERS.LOOKUP_DEF, 
                EXTENSION_CONSTANTS.FILES.WORKFIELDS_XML
            );
            
            if (!fs.existsSync(workfieldsPath)) {
                fs.writeFileSync(
                    workfieldsPath, 
                    EXTENSION_CONSTANTS.TEMPLATES.WORKFIELDS_CONTENT, 
